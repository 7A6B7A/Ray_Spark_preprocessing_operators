...
# One-click runner: 3 datasets x 1 StandardScaler job each, with 4-node monitoring.
# Assumptions:
# - Run on master node.
...
REMOTE_BASE="${REMOTE_BASE:-/tmp/spark_standard_scaler_exp}"
INTERVAL="${INTERVAL:-1}"

# StandardScaler options (default: withStd=true, withMean=false)
WITH_STD="${WITH_STD:-1}"
WITH_MEAN="${WITH_MEAN:-0}"
REPARTITION="${REPARTITION:-0}"  # 0 means no repartition
CACHE="${CACHE:-0}"              # 1 to cache features/scaled

RUNS_DIR="${RUNS_DIR:-$DIR/runs}"

# Extra experiment: time-only scaling for subset_300mb with 3/2/1 machines.
RUN_TIME_ONLY_EXP="${RUN_TIME_ONLY_EXP:-1}"
TIME_ONLY_DATASET_FILE="${TIME_ONLY_DATASET_FILE:-subset_300mb.csv}"
TIME_ONLY_MACHINES_LIST="${TIME_ONLY_MACHINES_LIST:-3 2 1}"
TIME_ONLY_EXECUTOR_CORES_PER_MACHINE="${TIME_ONLY_EXECUTOR_CORES_PER_MACHINE:-4}"
...
extract_spark_result_field() {
  # Usage: extract_spark_result_field <log_path> <python_expr>
  # python_expr evaluates in a scope containing `obj` (parsed JSON dict).
  local log_path="$1" expr="$2"
  "$PYTHON_BIN" - "$log_path" "$expr" <<'PY'
import json,sys
path=sys.argv[1]
expr=sys.argv[2]
obj=None
  ...
  with open(path,'r',encoding='utf-8',errors='ignore') as f:
    for line in f:
      if line.startswith('__RESULT_JSON__'):
        obj=json.loads(line[len('__RESULT_JSON__'):])
        break
...
run_time_only_experiment_subset_300mb() {
  local dataset_path="$DATA_DIR/$TIME_ONLY_DATASET_FILE"
  [[ -f "$dataset_path" ]] || die "Dataset not found: $dataset_path"

  local ts exp_id exp_dir out_csv
  ts="$(date '+%Y%m%d_%H%M%S')"
  exp_id="time_only_${TIME_ONLY_DATASET_FILE%.*}_$ts"
  exp_dir="$RUNS_DIR/$exp_id"
  mkdir -p "$exp_dir"

  out_csv="$exp_dir/time_only.csv"
  echo "dataset_file,machines,executor_instances,executor_cores,total_executor_cores,job_seconds,rows,run_id,rc" >"$out_csv"

  log "=== TIME-ONLY EXP $exp_id (dataset=$dataset_path) ==="

  local m
  for m in $TIME_ONLY_MACHINES_LIST; do
    local run_id log_path rc total_cores job_seconds rows
    run_id="${TIME_ONLY_DATASET_FILE%.*}_${m}m_$ts"
    log_path="$exp_dir/driver_${m}m.log"
    total_cores=$((TIME_ONLY_EXECUTOR_CORES_PER_MACHINE * m))

    local args=("--input" "$dataset_path" "--run-id" "$run_id")
    if [[ "$WITH_STD" == "1" ]]; then args+=("--with-std"); fi
    if [[ "$WITH_MEAN" == "1" ]]; then args+=("--with-mean"); fi
    if [[ "$REPARTITION" != "0" ]]; then args+=("--repartition" "$REPARTITION"); fi
    if [[ "$CACHE" == "1" ]]; then args+=("--cache"); fi

    log "[time-only] machines=$m executor.instances=$m executor.cores=$TIME_ONLY_EXECUTOR_CORES_PER_MACHINE total_cores=$total_cores"
    ...
    "$SPARK_SUBMIT_BIN" \
      ...
      --conf "spark.executor.instances=$m" \
      --conf "spark.executor.cores=$TIME_ONLY_EXECUTOR_CORES_PER_MACHINE" \
      $SPARK_DEPLOY_MODE_OPT \
      $SPARK_SUBMIT_EXTRA_OPTS \
      "$DIR/spark_standard_scaler_job.py" \
      "${args[@]}" \
      >"$log_path" 2>&1
    rc=$?
    set -e

    job_seconds="$(extract_spark_result_field "$log_path" "obj.get('timing',{}).get('job_seconds','')")"
    rows="$(extract_spark_result_field "$log_path" "obj.get('rows','')")"
    echo "$TIME_ONLY_DATASET_FILE,$m,$m,$TIME_ONLY_EXECUTOR_CORES_PER_MACHINE,$total_cores,$job_seconds,$rows,$run_id,$rc" >>"$out_csv"
  done

  log "Time-only results: $out_csv"
}
...
run_one_dataset() {
  ...
  if [[ "$WITH_STD" == "1" ]]; then job_args+=("--with-std"); fi
  if [[ "$WITH_MEAN" == "1" ]]; then job_args+=("--with-mean"); fi
...
main() {
  ...
  if [[ "$RUN_TIME_ONLY_EXP" == "1" ]]; then
    run_time_only_experiment_subset_300mb || true
  fi
